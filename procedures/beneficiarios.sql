CREATE OR REPLACE NONEDITIONABLE PROCEDURE "LAB_BANCO"."UPSET_BENEFICIARIOS"(
    b_cpf VARCHAR2,
    ...
    IF b_cpf IS NOT NULL THEN
        ...
        IF b_data_nascimento IS NOT NULL THEN
            BEGIN
                IF b_data_nascimento IS NOT NULL THEN
                    BEGIN
                        dt_nasc := TO_DATE(b_data_nascimento, 'DD/MM/YYYY');
                    EXCEPTION
                        WHEN OTHERS THEN
                            dt_nasc := NULL;
                    END;
        ...
        IF v_count = 0 THEN
            INSERT INTO beneficiarios (bnf_cpf, bnf_dat_nasc, bnf_deficiente_fisico, bnf_sex_id, bnf_rcs_id, bnf_mun_id) 
